"""Task analysis endpoints — execution, batch, cache, and history.

Orchestration-domain routers (pipelines, recommendations, metrics,
webhooks, scheduling) have been extracted to dedicated modules in
this package.
"""
...
def build_tasks_router(
    ...
    # Include extracted orchestration routers
    from src.api.pipelines import build_pipelines_router
    from src.api.recommendations import build_recommendations_router
    from src.api.metrics import build_metrics_router
    from src.api.webhooks import build_webhooks_router
    from src.api.scheduling import build_scheduling_router

    router.include_router(build_pipelines_router())
    router.include_router(build_recommendations_router())
    router.include_router(build_metrics_router())
    router.include_router(build_webhooks_router())
    router.include_router(build_scheduling_router())
